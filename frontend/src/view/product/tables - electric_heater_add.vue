<template>
  <div>
    ...
    <modalAddProduct :value="true" :propProductData="productData"></modalAddProduct>
    <Drawer
      :title="drawerTitle"
      v-model="drawer"
      width="720"
      ...
      <Form :model="productData">
        <Tabs v-model="drawerTabIndex">
            <Tab-pane label="产品基本信息">
              ...
              <Row :gutter="32">
                <Col span="12">
                  <FormItem label="APP平台" label-position="top">
                    <Select v-model="productData.appType" placeholder="选择需支持的APP类型" multiple>
                      <Option v-for="item in allAppTypeList" :value="item.id" :key="item.id">{{ item.name }}</Option>
                    </Select>
                  </FormItem>
                </Col>
                <Col span="12">
                  <FormItem label="APP借用SN8" label-position="top">
                    <Select v-model="productData.appCopySn8" placeholder="APP完全借用可选择借用机型的SN8，可选">
              ...
              <Row :gutter="32">
                <Col span="6">
                  <FormItem label="项目经理" label-position="top">
                    <Input :disabled="isUpdate" v-model="productData.pm" :maxlength="50" placeholder="填写项目经理姓名" clearable></Input>
                  </FormItem>
                </Col>
                <Col span="6">
                  <FormItem label="企划" label-position="top">
                    <Input :disabled="isUpdate" v-model="productData.planner" :maxlength="50" placeholder="填写产品企划姓名" clearable></Input>
                  </FormItem>
                </Col>
                <Col span="6">
                  <FormItem label="电控硬件工程师" label-position="top">
                    <Input v-model="productData.hardwareEngineer" :maxlength="50" placeholder="填写电控硬件工程师姓名" clearable></Input>
                  </FormItem>
                </Col>
                <Col span="6">
                  <FormItem label="电控软件工程师" label-position="top">
                    <Input v-model="productData.softwareEngineer" :maxlength="50" placeholder="填写电控软件工程师姓名" clearable></Input>
                  </FormItem>
                </Col>
              </Row>
            </Tab-pane>
            <Tab-pane label="电控信息">
              ...
              <Row :gutter="32">
                <Col span="12">
                  <FormItem label="加热管配置" label-position="top">
                    <Input v-model="productData.hasBuzzer" :maxlength="50" placeholder="请选择加热管配置" clearable></Input>
                  </FormItem>
                </Col>
                <Col span="12">
                  <FormItem label="漏电断电配置" label-position="top">
                    <Input v-model="productData.hasFlowSensor" :maxlength="50" placeholder="请选择是否有流量传感器" clearable></Input>
                  </FormItem>
                </Col>
              </Row>
            </Tab-pane>
            <Tab-pane label="APP功能">
              <Row :gutter="32">
                <Col span="12">
                  <FormItem label="产品功能" label-position="top">
                    ...
                    <Select v-model="functionIds1" placeholder="请选择产品功能" multiple>
                      <Option-group label="中间列表功能">
                        <Option v-for="item in getFunctionListByType('function')" :value="item.id" :key="item.id">{{ item.functionName }}</Option>
                      </Option-group>
                      <Option-group label="底部控制功能">
                        <Option v-for="item in getFunctionListByType('controlFunc')" :value="item.id" :key="item.id">{{ item.functionName }}</Option>
                      </Option-group>
                    </Select>
                  </FormItem>
                </Col>
              ...
              <Row :gutter="32">
                <Col span="12">
                  <FormItem label="镁棒显示" label-position="top">
                    <Select v-model="functionIds2" placeholder="请选择镁棒显示信息">
                      <Option v-for="item in getFunctionListByType('consumableMb')" :value="item.id" :key="item.id">{{ item.functionName }}</Option>
                    </Select>
                  </FormItem>
                </Col>
                <Col span="12">
                  <FormItem label="滤芯显示" label-position="top">
                    <Select v-model="functionIds3" placeholder="请选择滤芯显示信息">
                      <Option v-for="item in getFunctionListByType('consumableFilter')" :value="item.id" :key="item.id">{{ item.functionName }}</Option>
                    </Select>
                  </FormItem>
                </Col>
              </Row>
              <Row :gutter="32">
                <Col span="12">
                  <FormItem label="内胆显示" label-position="top">
                    <Select v-model="functionIds4" placeholder="请选择内胆显示信息">
                      <Option v-for="item in getFunctionListByType('consumableTank')" :value="item.id" :key="item.id">{{ item.functionName }}</Option>
                    </Select>
                  </FormItem>
                </Col>
                <Col span="12">
                  <FormItem label="内胆排水阀类型" label-position="top">
                    <Select v-model="productData.functionIdxx" placeholder="请选择内胆排水阀类型">
                      <Option v-for="item in functionList" :value="item.id" :key="item.id">{{ item.functionName }}</Option>
                    </Select>
                  </FormItem>
                </Col>
              ...
              <Row :gutter="32">
                <Col span="12">
                  <FormItem label="预约类型" label-position="top">
                    <Select v-model="functionIds5" placeholder="请选择预约类型">
                      <Option v-for="item in getFunctionListByType('appoint')" :value="item.id" :key="item.id">{{ item.functionName }}</Option>
                    </Select>
                  </FormItem>
                </Col>
              </Row>
      ...
      <div class="demo-drawer-footer">
        ...
        <Button v-if="drawerTabIndex != 0" style="margin-right: 8px" @click="drawerTabIndex-=1">上一步</Button>
        <Button v-if="drawerTabIndex == 3" type="primary" @click="saveProduct">保存</Button>
        <Button v-else type="primary" @click="drawerTabIndex+=1">下一步</Button>
      </div>
    </Drawer>
    <span>
      <Card>
        ...
        <Row :gutter="0">
          ...
          <Col span="5">
            <FormItem label="SN8:" :label-width="80">
              <Input v-model="productDataForSearch.productSN8" maxlength="50" placeholder="请输入产品SN8，8个字符"
                     clearable></Input>
            </FormItem>
          </Col>
          <Col span="5">
    ...
    <Card>
      ...
      <tables ref="tables" editable searchable search-place="" border stripe v-model="tableData" :columns="columns"
  ...
  methods: {
        ...
        if (this.accessProductType) {
          if (this.accessExport) {
            if (this.selectDatas.length > 0) {
              ...
              const params = {
                title: ['品类', '品牌', '产品编号', '产品型号', 'SN8', '功能', '场景', '语音功能', '生态入口', '传感器'],
                key: ['productTypeName', 'branchName', 'code', 'model', 'sN8', 'functionsName', 'scenariosNameSupported', 'voiceFunctionName', 'ecologyEntranceName', 'sensorName'],
    ...
    downloadImportTemplate () {
      ...
      */
      ...
      const params = {
        title: ['品类', '品牌', '产品编号', '产品型号', 'SN8', '功能', '场景', '语音功能', '生态入口', '传感器'],
        key: ['categoryName', 'branchName', 'code', 'model', 'SN8', 'functionsName', 'scenariosNameSupported', 'voiceFunctionName', 'ecologyEntranceName', 'sensorName'],
        data: [{
      ...
      store.dispatch('getUserInfo').then(user => {
        ...
        if (this.accessProductType) {
          if ((this.accessUpdate && booleanForUpdate) || (this.accessAdd && booleanForAdd)) {
            this.productData.functionIds = this.functionIds1.concat(this.functionIds2).concat(this.functionIds3).concat(this.functionIds4).concat(this.functionIds5).concat(this.functionIds6)
